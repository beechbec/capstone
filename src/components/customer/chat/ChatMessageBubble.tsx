  ...
  return (
    <div className={`mt-4 flex ${isBot ? 'justify-start' : 'justify-end'}`}>
      <div className="max-w-[85%] sm:max-w-[75%] lg:max-w-[60%]">
        <div
          className={`inline-block w-full break-words rounded-2xl px-3 py-3 sm:px-4 border shadow-sm text-left ${
            isBot 
              ? 'bg-white border-blue/20 text-blue' 
              : 'bg-blue text-white border-blue'
          }`}
        >
          <p className="text-sm sm:text-base leading-relaxed text-left">{text}</p>
        </div>
